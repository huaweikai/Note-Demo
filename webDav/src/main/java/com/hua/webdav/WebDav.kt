...
import com.hua.webdav.utils.baseUrl
import com.hua.webdav.utils.findNS
import com.hua.webdav.utils.findNSPrefix
import com.hua.webdav.utils.newCall
import com.hua.webdav.utils.toRequestBody
...
open class WebDav(
    ...
    companion object {
        ...
        @SuppressLint("DateTimeFormatter", "ConstantLocale")
        ...
        for (element in elements) {
            ...
            try {
                ...
                webDavFile = WebDavFile(
                    "https://${urlStr.baseUrl}/$hrefDecode",
                    authorization,
    ...
    suspend fun upload(byteArray: ByteArray, contentType: String) {
        ...
        kotlin.runCatching {
            withContext(IO) {
                ...
                val url = httpUrl ?: throw IllegalStateException("url不能为空")
                webDavClient.newCall {
                    url(url)
                    put(fileBody)
                }.use {
    ...
    @Throws(WebDavException::class)
    suspend fun upload(context: Context, uri: Uri, contentType: String) {
        // 务必注意RequestBody不要嵌套，不然上传时内容可能会被追加多余的文件信息
        kotlin.runCatching {
            withContext(IO) {
                val fileBody = uri.toRequestBody(context, contentType.toMediaType())
                val url = httpUrl ?: throw IllegalStateException("url不能为空")
                webDavClient.newCall {
                    url(url)
                    put(fileBody)
    ...
    suspend fun downloadInputStream(): InputStream {
        ...
        val byteStream = webDavClient.newCall {
            url(url)
        }.apply {
            checkResult(this)
        }.body?.byteStream()
